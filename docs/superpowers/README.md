...
Phase 5's two live library steps are ticked but marked **done differently**: no account here has a
plan that allows team library APIs, so the library half rests on stubbed unit tests.
`docs/guides/libraries.md` ("What has been verified against real Figma") lists what was never run
live.
